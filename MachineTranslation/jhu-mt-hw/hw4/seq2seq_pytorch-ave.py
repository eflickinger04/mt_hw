...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        # Get the hidden state from the previous time step
        h_t = hidden[0][-1]  # (batch_size, hidden_size)

        # Reshape embedded to (batch_size, hidden_size)
        embedded = embedded.squeeze(0)

        # Calculate attention weights
        attn_input = torch.cat((embedded, h_t), 1)  # (batch_size, hidden_size * 2)
        ...
        output = torch.cat((embedded, attn_applied), 1)  # (batch_size, hidden_size * 2)
        output = F.relu(self.attn_combine(output)).unsqueeze(0)  # (1, batch_size, hidden_size)

        # Run through LSTM
        output, hidden = self.lstm(output, hidden)  # output: (1, batch_size, hidden_size)

        # Output layer
        output = F.log_softmax(self.out(output.squeeze(0)), dim=1)  # (batch_size, output_size)
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_outputs, encoder_hidden = encoder(input_tensor, encoder_hidden)  # encoder_outputs: (seq_len, batch_size, hidden_size)
    # encoder_outputs = encoder_outputs.squeeze(1)  # (seq_len, hidden_size)
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            decoder_input = top_i.detach()  # Detach from history as input
            if decoder_input.item() == EOS_index:
                break
    ...
    with torch.no_grad():
        ...
        for di in range(max_length):
            ...
            if top_i.item() == EOS_index:
                ...
                decoded_words.append(tgt_vocab.index2word[top_i.item()])

            decoder_input = top_i.detach()
