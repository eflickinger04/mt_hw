...
class EncoderRNN(nn.Module):
    """The Encoder RNN class implemented with a manual LSTM (Bidirectional)."""
    def __init__(self, input_size, hidden_size):
        ...
        # bidirectional
        self.W_i_forward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_i_forward = nn.Linear(hidden_size, hidden_size)
        self.W_f_forward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_f_forward = nn.Linear(hidden_size, hidden_size)
        self.W_o_forward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_o_forward = nn.Linear(hidden_size, hidden_size)
        self.W_c_forward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_c_forward = nn.Linear(hidden_size, hidden_size)

        self.W_i_backward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_i_backward = nn.Linear(hidden_size, hidden_size)
        self.W_f_backward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_f_backward = nn.Linear(hidden_size, hidden_size)
        self.W_o_backward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_o_backward = nn.Linear(hidden_size, hidden_size)
        self.W_c_backward = nn.Linear(hidden_size, hidden_size, bias=False)
        self.U_c_backward = nn.Linear(hidden_size, hidden_size)

    def forward(self, input, hidden_forward, hidden_backward):
        embedded = self.embedding(input).view(1, -1)

        # fwd
        h_forward_prev, c_forward_prev = hidden_forward
        i_t_forward = torch.sigmoid(self.W_i_forward(embedded) + self.U_i_forward(h_forward_prev))
        f_t_forward = torch.sigmoid(self.W_f_forward(embedded) + self.U_f_forward(h_forward_prev))
        o_t_forward = torch.sigmoid(self.W_o_forward(embedded) + self.U_o_forward(h_forward_prev))
        c_tilde_forward = torch.tanh(self.W_c_forward(embedded) + self.U_c_forward(h_forward_prev))
        c_forward = f_t_forward * c_forward_prev + i_t_forward * c_tilde_forward
        h_forward = o_t_forward * torch.tanh(c_forward)

        # bkwd
        h_backward_prev, c_backward_prev = hidden_backward
        i_t_backward = torch.sigmoid(self.W_i_backward(embedded) + self.U_i_backward(h_backward_prev))
        f_t_backward = torch.sigmoid(self.W_f_backward(embedded) + self.U_f_backward(h_backward_prev))
        o_t_backward = torch.sigmoid(self.W_o_backward(embedded) + self.U_o_backward(h_backward_prev))
        c_tilde_backward = torch.tanh(self.W_c_backward(embedded) + self.U_c_backward(h_backward_prev))
        c_backward = f_t_backward * c_backward_prev + i_t_backward * c_tilde_backward
        h_backward = o_t_backward * torch.tanh(c_backward)


        h_concat = torch.cat((h_forward, h_backward), dim=1)
        return h_concat, (h_forward, c_forward), (h_backward, c_backward)
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.attn_combine = nn.Linear(self.hidden_size * 2 + hidden_size, hidden_size)  
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        hidden_expanded = hidden[0].view(1, 1, -1)
        concatenated = torch.cat((embedded, hidden_expanded), dim=2)
        attn_weights = F.softmax(self.attn(concatenated.view(1, -1)), dim=1)
        attn_applied = torch.bmm(attn_weights.unsqueeze(0), encoder_outputs.unsqueeze(0))

        output = torch.cat((embedded, attn_applied), dim=2)
        output = self.attn_combine(output.view(1, -1)).unsqueeze(0)
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    encoder_hidden_forward, encoder_hidden_backward = encoder.get_initial_hidden_state()
    ...
    for ei in range(input_length):
        encoder_output, encoder_hidden_forward, encoder_hidden_backward = encoder(
            input_tensor[ei], encoder_hidden_forward, encoder_hidden_backward)
        encoder_outputs[ei] = encoder_output

    decoder_input = torch.tensor([[SOS_index]], device=device)
    decoder_hidden = (encoder_hidden_forward[0] + encoder_hidden_backward[0], 
                      encoder_hidden_forward[1] + encoder_hidden_backward[1])
...
def translate(encoder, decoder, sentence, src_vocab, tgt_vocab, max_length=MAX_LENGTH):
    ...
    with torch.no_grad():
        ...
        for ei in range(input_length):
            ...
            encoder_outputs[ei] = encoder_output.squeeze(0)  # [1, 512] -> [512]

        for ei in reversed(range(input_length)):
            encoder_output, _, encoder_hidden_backward = encoder(
                input_tensor[ei], encoder_hidden_forward, encoder_hidden_backward)
